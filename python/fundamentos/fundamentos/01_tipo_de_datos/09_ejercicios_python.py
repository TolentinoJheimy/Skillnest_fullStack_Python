...
def Npares():
    lista_final = ""
    numLook = int(input("Cuantos numeros pares deseas ver? "))
...
def verificador():
    year = 2026
    yearLook = int( input("Cual es tu año de nacimiento? "))
...
def calDesc():
    final = 0.0
    productLook = float(input( "Cual es el precio de tu producto? (Dolares) "))
    if productLook >= 100:
...
def clasificador():
    numLook = int(input("Cual es tu numero? "))
...
def tabla():
    numlook = int(input( "Dime un numero entero: "))
...
def sumatoria():
    total = 0
    while True:
        num = int(input("Dame un numero (si quieres salir, pon un numero negativo) "))
        if num > 0:
            total += num
...
'''
def contador():
    frase = input("Dame una frase o palabra ").lower()
    vocales = "aeiouáéíóú" 
    veces = 0
...
def validador():
    ...
    for i in range(3):
        questLook = input("Adivina la contraseña ")
...
def registro():
    arreglo = []
    for i in range(5):
        arreglo.append(input(f"Nombre {i+1}: "))
    print("Inverso", arreglo[::-1])
registro()

def promedio():
    cantidad = int(input("Cuantas notas? "))
...
def simulador():
    ...
    precios = []
    for i in range(3):
        n = input(f"Producto {i+1} ")
        p = float(input(f"Precio de {n} "))
